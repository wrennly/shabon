...
export const SHABON_SHADER_SKSL = Skia.RuntimeEffect.Make(`
uniform float iTime;
...
vec3 paletteLight(float t) {
    ...
    vec3 b = vec3(0.25, 0.25, 0.25); // Increased saturation (was 0.15)
...
vec3 paletteDark(float t) {
    ...
    vec3 b = vec3(0.3, 0.3, 0.35); // Reduced from 0.5
    vec3 c = vec3(1.0, 1.0, 1.0);
...
vec4 main(vec2 fragCoord) {
    ...
    // Time factor
    float t = iTime * 0.15; // Slower movement
    ...
    // Start from 0.35 (closer to edge) to 0.5 (edge)
    // Narrowed range to reduce rainbow dominance
    // Apply iRainbowStrength to control visibility
    float rainbowMask = smoothstep(0.35, 0.5, dist) * iRainbowStrength;
    
    // Sharper edge for opacity/white glow
    float fresnel = pow(smoothstep(0.4, 0.5, dist), 3.0); 
    ...
    // Increased mix strength to make rainbow visible against white
    vec3 baseColor = mix(centerColor, rainbow, rainbowMask * 0.9);
    ...
    // Increased base opacity for better visibility (0.15 base + noise)
    alpha += (0.15 + f * 0.05) * iFillAlpha; 
    
    // Rainbow visibility: If there is a rainbow, we need alpha to show it!
    // Reduced slightly to make it less intense
    alpha += rainbowMask * 0.2;
    
    // Edges are visible but translucent
    // Scale by max(iRainbowStrength, iFillAlpha) to ensure it disappears when both are 0
    alpha += fresnel * 0.5 * clamp(iRainbowStrength + iFillAlpha, 0.0, 1.0);
    ...
    // Reduced intensity to let rainbow show through
